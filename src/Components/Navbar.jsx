  ...
  return (
    ...
    <nav className="flex justify-between items-center px-8 py-5 shadow-md">
      ...
      <ul className="hidden md:flex gap-8 font-medium">
        ...
        <li className='mr-6'>
          <Link to="/event" className='hover:text-blue-500'>Event Organized</Link>
        </li>
      </ul>
      <button onClick={() => setDarkMode(!darkMode)} className="px-4 py-2 rounded-lg bg-blue-500 text-white">
        {darkMode ? "☀ Light" : "🌙 Dark"}
      </button>
    </nav>
  );
